...
- Verdi X-2025.06 for linux64（2025-05-31 build）安裝在 `/apps/cad/verdi/verdi/2025.06`；同層的 `cur` 是指向該版本的 symlink。公開根是 `/apps`，套件在 `/apps/cad`。
...
- 2026-09-14 起有 `/apps/bin/verdi`（source `verdi.sh` 再 exec）。`/apps/bin/vcs` 仍**只** source `vcs.sh`，與舊 `/usr/local/bin/vcs` 相同限制。舊 `/usr/local/bin/{vcs,spyglass,sg_shell}` 已刪。
...
```bash
source /apps/cad/synopsys/CIC/vcs.sh
source /apps/cad/synopsys/CIC/verdi.sh
...
- 編譯輸出可看到 VCS 自動連結 `/apps/cad/verdi/verdi/2025.06/share/PLI/VCS/LINUX64/pli.a`，並印出 `Verdi KDB elaboration done and the database successfully generated`。
...
- 只用只 source `vcs.sh` 的 launcher（舊 `/usr/local/bin/vcs`，現在的 `/apps/bin/vcs` 一樣）編譯含 FSDB dump 的 testbench 會失敗：VCS 回報 `Undefined System Task call to '$fsdbDumpfile'`、`Undefined System Task call to '$fsdbDumpvars'`，編譯結束碼 255。
- 根因不是 VCS：`vcs.sh` 內把 `VERDI_PLI` 指向 `/apps/cad/design_compiler/verdi/cur/share/PLI/VCS/LINUX64`，而該路徑在此安裝中不存在，所以 `Verdi PLI linked.` 這行永遠不會出現。VCS 只有在 `VERDI_HOME` 被設定時才會找到 Verdi PLI，因此實際上是靠 `verdi.sh` 補上。
...
- Verdi 安裝內自帶 `Xvfb`：`/apps/cad/verdi/verdi/2025.06/bin/Xvfb`（symlink 到 `.wrapper`，實際 binary 在 `platform/LINUX64/bin/Xvfb`）。以 `.../bin/Xvfb :99 -screen 0 1920x1080x24 -nolisten tcp -fp /usr/share/fonts/X11/misc` 啟動後，`DISPLAY=:99 verdi …` 可完整跑起 nTrace／nWave／nSchema，不需要 root 安裝任何套件。
