...
Before a cutover, run `PRAGMA quick_check` on each database and verify that no process still points at the old shared writable state. During a recent inventory, shared Codex seed artifacts were not valid SQLite databases (`file is not a database`); treat these files as invalid seed/artifact evidence and use the host-local runtime state instead. Do not repair or synchronize them in place.
