...
- 2026-09-15 11:42 使用者明確要求啟動後，已建立並啟動 Mazu `canonical-repack-20260915.service`（runtime unit，沒有自動重啟）。工作目錄 `/usr/2TB-SSD/backup-work/canonical-repack-20260915` 的 `repack.py` 以原組與補組的完整 tar member 區段串流重建；保留原始 PAX／ACL／xattr／sparse 編碼，略過已確認 EDA 範圍與特殊節點，尾端追加 19,853 筆歷史硬連結及目錄 metadata 重播，再正常結束 tar。預計保留 24,480,914 個唯一項目；重複的收尾 link／directory records 是標準 tar 可處理的最終狀態更新，不需舊 recovery helper。功能測試已實際涵蓋截斷原串流、精確排除、長 PAX 路徑、稀疏檔、xattr、ACL、數字 UID/GID、跨組硬連結、目錄奈秒時間、分卷搬移與選定參數的 round-trip；沒有做效能 benchmark。這是啟動紀錄，並非新組完成。
  新卷先在 SSD `volumes/`；SSD 剩餘低於 250 GiB 時暫停 encoder，只複製已 finalized 且沒有開啟 fd 的完整數字卷（不搬第一卷或 `.tmp`），到常備碟 `backup/home/Canonical-Home.repacked-20260915.incomplete`，fsync 與 SHA-256 讀回吻合後，才用 symlink 原子替換 SSD 副本。常備碟保留 100 GiB；不足時停止，不使用系統碟。7z 26.03 的功能測試確認這種搬移後可正常完成並解壓；預先建立未來分卷 symlink 會因 EEXIST 失敗，不能使用。`progress.json` 為主進度，`storage-progress.json` 顯示暫停搬卷，`failure.json` 為失敗；只有完整新組 decoder/CRC、逐檔 SHA／metadata、保留清單、收尾連結／目錄、全 tar SHA／長度、分卷序號／大小／SHA 通過才寫 `complete.json`。`volumes.json` 記錄分散位置；完成後仍需經確認才能清理舊組並歸位新卷。停止時 `ExecStopPost` 恢復常備碟唯讀；原 48 卷、補 11 卷及舊 metadata 未刪改。開發來源留在 `<worktree-root>/canonical-repack-20260915`（現場 operational scripts，非 Git repository）。
