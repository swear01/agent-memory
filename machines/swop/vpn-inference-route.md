...
後續依使用者要求將單機 /32 擴為經伺服器介面與路由確認的實驗室 LAN /24，移除被涵蓋的 /32。保留原有校網與 VPN 用戶網段，維持 SplitTunneling 開啟。重新連線後驗證兩台 LAN 服務 HTTP 200，且一般公網目的地仍走 Wi-Fi。使用者偏好實驗室網段分流；官方 latest 安裝包目前反而指定 SplitTunneling 為 false，重新安裝前須注意這項差異。
