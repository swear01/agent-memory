...
- 控制台管理與權限：utux 無對外 SSH 埠，由公開伺服器（如 zeus）透過 ProxyJump 連線至內網位址；本地端 SSH 設定已配置 ProxyJump。伺服器管理依賴本機 FIFO 控制台指令 `sudo /usr/local/sbin/vault-hunters-console '<command>'`，可熱執行 `list`（查在線玩家）、`whitelist add <ID>`（加白名單）及 `op <ID>`（授予權限）；因指令透過具名管道單向寫入，執行回應需由 `journalctl -u vault-hunters -n <行數>` 或 `logs/latest.log` 查驗。目前伺服器白名單已擴充至多位朋友（含 `Cell_phone_9914` 等），且在線的 `swear01` 與 `Cell_phone_9914` 皆已設為 OP（權限等級 4，持久化於 `ops.json`）。
