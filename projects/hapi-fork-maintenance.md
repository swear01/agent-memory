...
The `swop` Pi installation now uses the fleet-style model restriction: `settings.json` supplies the enabled-model list, `pi-model-filter@0.1.2` applies a default-block filter, and `strict-model-allowlist.ts` enforces the same allowlist at runtime. The seven configured identifiers are `opencode-go/deepseek-v4-pro`, `opencode-go/deepseek-v4-flash`, `openai-codex/gpt-5.6-luna`, `openai-codex/gpt-daybreak-blue-latest`, `openai-codex/gpt-5.6-terra`, `meta/muse-spark-1.2-contributor`, and `valkyrie-ninfer/qwen3.8-27b`; NInfer is the current default. Muse requires the unpinned `pi-meta-oauth` package as well as Meta OAuth or `META_API_KEY`/`MODEL_API_KEY`. On `swop`, `pi-meta-oauth` 0.5.0 is installed and Muse is present in all three restriction layers, but Meta authentication is not yet configured, so Muse remains absent from the available-model picker until `/login meta` completes. Distinguish a real stdout model row from `No models match pattern` on stderr: grepping combined `pi --list-models` output produced a false-positive Muse verification during onboarding. The interactive picker currently exposes Luna, Terra, and NInfer; the other configured identifiers lack an available catalog/auth match. Supplying a disallowed custom model on the command line can render its label before the first authenticated request, so verify restrictions through the interactive picker plus extension-load diagnostics and preserve `auth.json` when deploying configuration files.
