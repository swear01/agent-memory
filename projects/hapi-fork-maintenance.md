...
Pi now uses the canonical fleet model restriction: `settings.json` supplies the enabled-model list, `pi-model-filter@0.1.2` applies a default-block filter, and `strict-model-allowlist.ts` enforces the same allowlist at runtime. The five allowed identifiers are `opencode-go/deepseek-v4-pro`, `opencode-go/deepseek-v4-flash`, `openai-codex/gpt-5.6-luna`, `openai-codex/gpt-daybreak-blue-latest`, and `openai-codex/gpt-5.6-terra`, with Luna as the default. On this host, Pi's scoped and all-model interactive pickers expose only Luna and Terra because the other three identifiers do not currently match its model catalog; a search for `gpt-5.5` returns no selectable model. The non-interactive `pi --list-models` command prints the raw registry before extension filtering, so it is not a valid allowlist verifier. Supplying a disallowed custom model on the command line can also render its label before the first authenticated request; runtime rejection was not exercised because that would send an API request. Verify restrictions through the interactive picker plus extension-load diagnostics, and preserve `auth.json` when deploying the three configuration files.
