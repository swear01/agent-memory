...
| Context-preserving fleet v2.1 | reprocessed, review pending | A field/span-preserving sanitizer retained safe sibling context, HAPI messages, nested lesson text, and non-secret SHA provenance while removing dangerous spans. After recursive residual-secret scanning, 4,384 source files yielded 366,256 records seen, 366,011 written, 244 skipped, and 1 empty record (99.93% retained). Final JSONL-safe scans passed on all seven machine corpora; seven fresh machine-scoped reviews are pending. NFS duplicate views remain one representative. |
